...
export class Post extends Component<PostRouteProps, PostState> {
  ...
  trackCommentsBoxScrolling = () => {
    ...
    if (wrappedElement && this.isBottom(wrappedElement)) {
      const commentCount =
        this.state.commentsRes.state === "success"
          ? this.state.commentsRes.data.data.length
          : 0;
  ...
  handleViewContext(): string {
    if (this.state.commentsRes.state === "success") {
      ...
      const commentView = this.state.commentsRes.data.data.find(
  ...
  async handleLockComment(form: LockComment) {
    ...
    this.setState(s => {
      if (res.state === "success" && s.commentsRes.state === "success") {
        s.commentsRes.data.data = editCommentsSlimLocked(
  ...
  createAndUpdateComments(res: RequestState<CommentResponse>) {
    this.setState(s => {
      if (s.commentsRes.state === "success" && res.state === "success") {
        ...
        const comments = s.commentsRes.data.data;
  ...
  findAndUpdateCommentEdit(res: RequestState<CommentResponse>) {
    this.setState(s => {
      if (s.commentsRes.state === "success" && res.state === "success") {
        s.commentsRes.data.data = editCommentSlim(
