...
function processModlogEntry({
  type_,
  ...view
}: ModlogCombinedView): ModlogEntry {
  switch (type_) {
    case "AdminAllowInstance": {
      const {
        ...
        admin,
        instance: { domain },
      } = view as AdminAllowInstanceView;
    ...
    case "AdminBlockInstance": {
      const {
        ...
        admin,
        instance: { domain },
      } = view as AdminBlockInstanceView;
      ...
      return {
        ...
        data: (
          <>
            ...
            <span>
              <PersonListing person={other_person} />
            </span>
      ...
      return {
        ...
        data: (
          <>
            ...
            {expires_at && (
              <span>
                <div>expires: {formatPastDate(expires_at)}</div>
              </span>
            )}
          </>
        ),
    ...
    case "ModBanFromCommunity": {
      const {
        ...
        community,
        other_person,
      } = view as ModBanFromCommunityView;
    ...
    case "ModChangeCommunityVisibility": {
      const {
        ...
        moderator,
        community,
      } = view as ModChangeCommunityVisibilityView;
      ...
      return {
        ...
        data: (
          <>
            ...
            <span>
              {" "}
              by <PersonListing person={other_person} />
            </span>
            {reason && (
              <span>
                <div>reason: {reason}</div>
    ...
    case "ModRemoveCommunity": {
      const {
        ...
        moderator,
        community,
      } = view as ModRemoveCommunityView;
...
export class Modlog extends Component<ModlogRouteProps, ModlogState> {
  ...
  get combined() {
    ...
    return (
      <tbody>
        {combined.map(i => {
          const { id, moderator, publishedAt, data } = processModlogEntry(i);
            ...
            <tr key={id}>
              ...
              <td>
                {this.amAdminOrMod && moderator ? (
                  <PersonListing person={moderator} />
                ) : (
